...
def sign_url(request):
    ...
    if(userPermission == 'noPermission'):
        return django.http.HttpResponse(json.dumps({'permission':userPermission}))
    url = "http://"+ bucket + "." + HOST  + "/"+object
    resource = "/" + bucket + "/" + object
    url_with_auth = oss.sign_url_auth_with_expire_time(method, url, headers, resource, TIMEOUT)
    return django.http.HttpResponse(json.dumps({'permission':userPermission,'url_with_auth':url_with_auth}))
...
def list_bucket(request):
    ...
    if(bucket):
        ...
        if (res.status / 100) == 2:
            ...
            return django.http.HttpResponse(json.dumps(object_list))
    return django.http.HttpResponse(False)
